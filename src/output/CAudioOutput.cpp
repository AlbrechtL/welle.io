...
CAudioOutput::CAudioOutput(std::shared_ptr<CRingBuffer<int16_t>> Buffer)
{
    AudioOutput = NULL;
    CardRate = 0;
    this->Buffer = Buffer;
...
CAudioIODevice::CAudioIODevice(std::shared_ptr<CRingBuffer<int16_t>> Buffer, QObject* parent)
    : QIODevice(parent)
{
    this->Buffer = Buffer;
}
