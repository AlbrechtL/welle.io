...
GstPadProbeReturn FileSinkCallbacks::UnlinkCallFileSink(GstPad *pad, GstPadProbeInfo *info, gpointer container_ptr) {
    FileSink::Data *data = reinterpret_cast<FileSink::Data *>(container_ptr);
...
void FileSink::InitSink(void *player_data) {
    if(linked())
        return;
...
void FileSink::Finish() {
    ...
    gst_pad_add_probe(data_->teepad, GST_PAD_PROBE_TYPE_IDLE, FileSinkCallbacks::UnlinkCallFileSink, data_, NULL);
}
