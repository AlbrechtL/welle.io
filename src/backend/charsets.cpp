...
/*
 * A link error:
 *   std::codecvt<char16_t,char,struct _Mbstatet>::id
 *
 * Fixed by using QtString instead.
 */
#if defined(_WIN32)
#include <QString>
        ...
        CharacterSet charset, size_t num_bytes)
{
    ...
    length = (num_bytes == 0) ? qstrlen((const char*)buffer) : num_bytes;
    ...
    switch (charset)
    {
       ...
       case CharacterSet::UnicodeUtf8:
            s = QString::fromUtf8 ((const char*)buffer, length);
            break;
