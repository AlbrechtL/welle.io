...
{
    mp2Decoder = std::make_unique<MP2Decoder>(this);
...
void Mp2Processor::addtoFrame(uint8_t *v)
{
    ...
    mp2Decoder->Feed(data, length);

    if (mp2File)
        fwrite(data, length, 1, mp2File.get());
};

void Mp2Processor::FormatChange(const std::string &format)
{
    std::clog << "mp2processor: " << format << std::endl;
}

void Mp2Processor::StartAudio(int samplerate, int channels, bool float32)
{
...
void Mp2Processor::PutAudio(const uint8_t *data, size_t len)
{
...
void Mp2Processor::ProcessPAD(const uint8_t *xpad_data, size_t xpad_len, bool exact_xpad_len, const uint8_t *fpad_data)
{
